...
Executes a Hamster Studio brief. **Execution-only**: the plan — parent tasks, subtasks, context — was generated upstream in Hamster Studio and synced into `.hamster/`. This skill schedules those existing tasks into parallel waves and executes them. It never generates, splits, elaborates, or replans tasks. If a task seems unclear, implement it as written.
...
Wait for all to complete; collect each executor's file list.
