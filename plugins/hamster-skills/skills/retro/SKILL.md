...
If `.hamster/` exists, write the current metrics as JSON for future comparisons:
...
If `.hamster/` is missing, skip this step.
...
| Error | Recovery |
|-------|----------|
| `.hamster/` missing | Skip snapshot write; still produce the narrative |
...
- The snapshot is saved to `.hamster/retros/` when that directory exists; otherwise skip it
